...
class AjaxTests(TestCase):
    ...
    def test_tilmeldingslister_signup_success(self):
        ...
        # Since this is the first pair and we have capacity for 24,
        # it should be assigned parnummer 1 and not be on the waiting list
        self.assertEqual(pair.parnummer, 1)
        self.assertFalse(pair.på_venteliste)
    ...
    def test_tilmeldingslister_invalid_data(self):
        ...
        data = json.loads(response.content)
        self.assertFalse(data['success'])
        self.assertIn('navn', data['error'].lower())
