...
class NotificationTests(TestCase):
    ...
    def test_email_on_assignment_update(self):
        ...
        response = self.client.post(
            reverse('meld_afbud'),
            {'list_id': self.substitutliste.id, 'substitut_id': self.assignment.user.id, 'responsible_email': 'admin@test.com'},
        ...
        self.assertIn(str(self.substitutliste.day.date()), email.body)
    ...
    def test_no_email_on_invalid_assignment(self):
        ...
        response = self.client.post(
            reverse('meld_afbud'),
            {'list_id': 999, 'substitut_id': self.assignment.user.id, 'responsible_email': 'admin@test.com'},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest'
        )
    ...
    def test_no_email_on_already_unavailable(self):
        ...
        self.assertTrue(response.json()['success'])
        # Current implementation still sends an email
        self.assertEqual(len(mail.outbox), 1)
